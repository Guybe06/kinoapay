/// Chaînes de caractères pour la feature Plus d'options.
class PlusStrings {
  static const String title = "Plus d'options";
  static const String subtitle =
      "Gérez vos outils de paiement et votre compte.";

  static const String actionContacts = "Contacts";
  static const String descContacts = "Vos contacts de paiement";

  static const String actionScan = "Scanner";
  static const String descScan = "Payer via QR Code";

  static const String actionRequest = "Demander";
  static const String descRequest = "Recevoir de l'argent";

  static const String actionProfile = "Mon Profil";
  static const String descProfile = "Gérer vos informations";

  static const String actionChannels = "Mes Canaux";
  static const String descChannels = "Modes de paiement";

  static const String actionHistory = "Historique";
  static const String descHistory = "Suivre vos activités";

  static const String sectionActions = "Actions Rapides";
}
